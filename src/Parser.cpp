	...
	if (!conf){
		...
		while(!conf.eof()){
			...
			if (item.compare("GCIndex")==0)
				...
				path = tempStr;
			else if(item.compare("genomeBed")==0)
				bedfile = tempStr;
		...
		if (bedfile=="")
		{
			cerr<<"Please provide a valid path for the bed formatted <chr, st, end, gene_id> annotation file"<<endl;
		}
        if (pairedEnds) {
            if (t == BED) {
