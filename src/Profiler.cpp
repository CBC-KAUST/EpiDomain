...
void Profiler::build_single_profile(vector<DNA_fragment >& tags, string chr, bool targetORcontrol){
	...
	for(vector<DNA_fragment>::iterator fragment_it=tags.begin(); fragment_it !=tags.end();++fragment_it ){
        ...
        if (((*fragment_it).fragmentLength==0 || (*fragment_it).fragmentLength>right) && isChIPseq_) { //single end or should be considered as SE due to a strange mapping: // @suppress("Suggested parenthesis around expression")
...
void Profiler::build_profile(map<string,vector<DNA_fragment > >& data, map<string,vector<DNA_fragment > >& control, string& path,
	...
	for (chr_it = data.begin();chr_it!=data.end();++chr_it){
		...
		if (hasControl_) build_single_profile(control[(*chr_it).first],(*chr_it).first,false);
	}
	...
	if (isInput) {
        ...
        for (map<string,vector<DNA_fragment > >::iterator chr_it = tags.begin();chr_it!=tags.end();++chr_it) {
            ...
            for (int bin_index=0; bin_index<bins_count; bin_index++) {
                ...
                read_count[(*chr_it).first][bin_index]=sum_of_elems;
...
void Profiler::call_peaks(int mergeDist){
	...
	if (!hasControl_) {
        ...
        if (elCount==0) {cerr << "Error: in the evaluation of lambda did not find any density values; please contact the Epidomain developers\n"; exit(-1);}
        lambda =  sum_of_elems/elCount;
        densities.clear();
        cout << "Evaluated lambda for the Poisson test: "<<lambda<<endl;
        if (lambda==0) {
            cerr << "Error: cannot continue with lambda equal to zero\nAbort!\nPlease contact the Epidomain developers"<<endl;
        }
	}
...
void Profiler::clean_reads(std::map<std::string,std::vector<DNA_fragment > >& data,
	...
	v3.resize(b-v3.begin()); // @suppress("Invalid arguments")
...
void Profiler::print_bedgraph(string name){
	...
	ChIP<<"track type=bedGraph name=\""<<name+"_ChIP density"<<"\"description=\"Generated By Epidomain\""<<
			"visibility=full color=200,100,0 altColor=0,100,200 priority=20"<<endl;

	Control<<"track type=bedGraph name=\""<<name+"_Input density"<<"\"description=\"Generated By Epidomain\""<<
				"visibility=full color=200,100,0 altColor=0,100,200 priority=20"<<endl;
